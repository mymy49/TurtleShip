...
#if USE_GUI
/*
#include <gui/FrameBufferArgb1555.h>
...
FrameBufferArgb1555::FrameBufferArgb1555(void)
{
}
...
uint8_t FrameBufferArgb1555::getPixelCapacity(void)
{
	return 2;
}

void FrameBufferArgb1555::drawDot(int16_t x, int16_t y)
{
...
void FrameBufferArgb1555::fillDotArray(uint32_t offset, uint32_t count, Color color)
{
	memsethw(&mFrameBuffer[offset], color.getCode(), count * 2);
}

Size FrameBufferArgb1555::getCanvasSize(void)
{
	return mSize;
}

void FrameBufferArgb1555::drawBitmapBase(Size canvasSize, Rectangular canvasRect, Position bitmapPos, const bitmap_t bitmap)
{
	switch(bitmap.type)
	{
...
void FrameBufferArgb1555::drawBitmapRgb565(Size canvasSize, Rectangular canvasDesArea, Position bitmapPos, const bitmap_t bitmap)
{
...
void FrameBufferArgb1555::drawBitmapArgb1555(Size canvasSize, Rectangular canvasDesArea, Position bitmapPos, const bitmap_t bitmap)
{
...
bitmap_t FrameBufferArgb1555::getBitmap(void)
{
	bitmap_t bitmap = 
	{
		mSize.getWidth(),		//uint16_t width;
		mSize.getHeight(),		//uint16_t height;
		BITMAP_TYPE_ARGB1555,	//bitmapType_t type; // 0 : RGB565, 1 : RGB888, 2 : ARGB1555
